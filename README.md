...
- 新增持久化运维日志，记录关键动作的结果、响应码、执行耗时和安全诊断元数据，并可在后台筛选或下载。
...
- 在“知识库管理”中维护人工知识：适用问题、检索关键词和知识内容仍兼容保存到 `content.json`。
...
- 通过后台运维日志追踪登录、人工知识、文件知识、模型、主持控制和问答调用的执行结果。
...
当前提交自动测试共 37 项，覆盖管理密码与会话、运维日志持久化/脱敏/轮转、主持词持久化、模式切换、确定性 SSE 指令、主持/问答互斥、知识文件导入与重启恢复、DOCX/PDF 提取、候选配置回滚、回答契约、模型配置安全、内容热加载、错误脱敏、数字人状态机和视频 Range 请求。

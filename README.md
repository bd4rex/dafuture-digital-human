...
- 应用版本：`0.3.0`。
- 已实现内容工作台、模型配置、问答接口、四态透明视频前台和浏览器语音播报。
- 模型 API Key 仅保存在被 Git 忽略的服务端配置文件中，接口和页面均不回显明文。
- 仓库不含真实模型配置，首次运行后需在工作台中自行填写。
- 当前为 MVP 原型，尚未部署到正式服务器，真人数字人素材和生产级 TTS 仍需后续接入。
...
- `http://127.0.0.1:8080/health`：服务健康状态。
...
- 支持内容热加载、原子保存、版本冲突检测、本机同源限制和可选管理密钥。
...
当前自动测试共 21 项，覆盖模型配置安全、问答调用、内容热加载、访问控制、错误脱敏、数字人状态机和视频 Range 请求。

## 公开仓库安全边界

- 请勿提交 `answer-mvp/model-config.json`、`.env`、私钥、证书私钥或真实 API Key。
- 公开访问后台前必须设置强随机 `ADMIN_API_KEY`，并由反向代理提供 HTTPS、限流和适当的日志策略。

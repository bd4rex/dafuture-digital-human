...
当前自动测试共 34 项，覆盖管理密码与会话、主持词持久化、模式切换、确定性 SSE 指令、主持/问答互斥、知识文件导入与重启恢复、DOCX/PDF 提取、候选配置回滚、回答契约、模型配置安全、内容热加载、错误脱敏、数字人状态机和视频 Range 请求。

## 公开仓库安全边界

- 请勿提交 `answer-mvp/model-config.json`、`admin-auth.json`、`host-scripts.json`、`knowledge.json`、`knowledge-files/`、`.env`、私钥或真实 API Key。

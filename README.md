...
- 小知识库在 24,000 字符预算内全量提供给模型；较大知识库先由已配置模型改写同义问法，再检索原始资料，仍无匹配时自然兜底。历史问答不再隐式参与回答，可从知识库页面迁入。
...
- 新版带背景 MP4 合计约 2.03 MB，主持状态使用 2026-09-13 的独立素材，支持真人海报与点击重试；手机首屏保留输入框，适配滚动对话、键盘和横屏。四态调试入口为 `/avatar?preview=1`，微信仍需真机验收。
...
- 当前数字人视频使用用户提供的男性形象及背景，原始母版保留在项目之外；旧透明素材仅供缓存兼容和显式回退。

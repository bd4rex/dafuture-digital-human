...
2026-09-15 播放修复补充：仓库现在包含前端 `speechProviders.server`，调用扣子部署已有的 `POST /api/tts`（`{text}` → `audio/*` 二进制）；仓库本身没有新增该后端或服务端 ASR。扣子已通过平台内置功能接好 TTS/ASR，应保留，不能按下面的历史建议重新搭建或改成其他接口。定向合并方式、取消策略与验收见 [扣子语音修复交接](COZE_SPEECH_FIX.md)。下面的供应商后端、持久化及预生成接口仍为设计建议。
...
- 当前语音：仓库默认使用浏览器识别/合成，前端另有 `server` 播放实现；扣子部署保留其内置 TTS/ASR。浏览器模式固定指定男声；服务端模式失败转文字，不自动切成浏览器声音。
...
| 知识检索 | 小库在 24,000 字符预算内全量上下文；大库增加一次同模型问法改写，再匹配原始资料；无证据自然兜底 | 保持两步日志与降级，不新增 Embedding 依赖；改写不作为事实 |
...
`speechProviders.server` 已在前端实现；仓库的 `voiceInputProviders.server` 和平台后端仍由部署方接入，扣子现有实现须保留。复用现有入口，不写第二套聊天状态机。

### 对话音频

1. 保留 `askQuestion` → `answerReady` 的流程；拿到答案后仍处于 `thinking/audio-preparing`。
2. 通用后端设计建议以该轮 `turnId` 请求 TTS；当前扣子适配器保留已有的 `/api/tts` 与 `{text}` 契约，返回音频后生成 Blob URL，不在本次修复中重写平台后端。
...
5. 禁止全局按键处理改写 `<audio>.src`。空闲手势解锁使用 Blob 静音音频，并检查“空闲／单个在途尝试／尝试身份”；取消先作废身份，旧 Promise 不得暂停或清空新音频。正式播放的 `playing` 同样标记音频已解锁。
6. 服务端 TTS 失败只保留文字、报告失败并恢复控件，不调用 `speakWithBrowser`，无论错误发生在合成前、启动时还是播报途中。指定男声名单不等于允许中途切换播报引擎。
...
- 主持新指令、后台停止、切换模式、主动静音及控制断线，继续沿用明确的取消语义。2026-09-15 起访客的新问题不能打断当前回答：生成、准备音频和播放期间只允许编辑草稿；结束后由用户发送，不自动排队。外部适配器必须保留 `canStartQuestion` 入口守卫和控件恢复路径。
...
第一阶段只做“一次说完，再识别”，不默认实现持续监听、唤醒词、回声消除或边播边打断。当前回答结束前禁用麦克风并守卫启动入口，不因录音操作停止播报；结束后才允许主动录音。识别最终文字后复用现有提交流程，保留键盘输入；取消/上一轮录音的迟到结果不得覆盖新草稿或触发提交。
...
| TTS 失败但答案有效 | 保留答案文字；仅在指定男声可用且符合音色策略时尝试浏览器语音，否则明确提示，不自动切换女声或未知音色 |

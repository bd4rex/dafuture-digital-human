...
当前管理页不再显示人工问题录入，但服务仍兼容已有 `content.json` 和 `GET/PUT /api/content`，不会迁移或删除现有内容。需要维护这部分兼容数据时可直接编辑文件；服务默认每 2 秒检查一次：

- 新文件校验成功时，整份内容一次性切换。
- 新文件格式错误时，继续使用上一份有效内容，并把健康状态标记为 `degraded`。
- 文件修复后自动恢复为 `ready`。

每条兼容内容仍需要唯一 `id`、非空 `questions`、非空 `keywords` 和 `answer`。这些字段继续参与模型上下文与访客快捷问题，但不再作为后台录入表单展示。

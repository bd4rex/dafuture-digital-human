...
当前 34 项测试覆盖管理密码首次设置、登录与退出、加盐哈希和同源保护、主持词持久化、模式切换、确定性 SSE 指令、主持模式问答互斥、知识文件导入/去重/替换/删除、重启恢复、DOCX/PDF 文字提取、知识上下文检索、候选模型配置失败回滚、Key 不回显、上游错误脱敏、内容热加载、数字人状态机和视频 Range 请求。
...
启动后打开管理页设置密码。命名卷同时持久化手工内容、主持词、模型配置、管理密码哈希、知识索引和原文件。不要把含真实 Key 的卷导出到公开位置。

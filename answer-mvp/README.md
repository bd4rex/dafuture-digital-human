...
当前提交的 37 项测试覆盖管理密码首次设置、登录与退出、加盐哈希和同源保护、运维日志持久化/筛选/下载/脱敏/轮转、主持词持久化、模式切换、确定性 SSE 指令、主持模式问答互斥、知识文件导入/去重/替换/删除、重启恢复、DOCX/PDF 文字提取、知识上下文检索、候选模型配置失败回滚、Key 不回显、上游错误脱敏、内容热加载、数字人状态机和视频 Range 请求。

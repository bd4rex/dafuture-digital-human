...
- `仅依据后台内容`：业务事实只能基于知识库列表中已导入的文件回答；资料不足时返回后台配置的“知识不足话术”。
- `允许补充一般知识`：优先使用后台内容，可以补充通用知识，但不得编造本项目专属的日期、地点、费用、人员或规则。

无论选择哪种模式，文件知识都只是模型的知识上下文，不会被接口直接当作最终答案返回。历史 `content.json` 不再隐式参与回答。

正常回答由模型依据“回答风格”自然组织。业务问题的模型结果为 `no_answer` 时，服务端会忽略模型自行编写的拒答内容，改用管理员配置的知识不足话术。模型未配置、连接失败、超时或响应异常时，接口保持相应的 HTTP 错误状态和错误码，但数字人前台显示并播报服务异常话术，技术原因只留在运维日志中。

常见纯问候、感谢和告别（例如“你好”“在吗”“谢谢”“再见”）不属于知识查询：服务端整句识别后只调用一次回答模型，不携带知识片段，也不做同义改写；仍使用后台的角色与回答风格。模型误返回有效 `no_answer` 时使用简短礼貌备份，标记 `answerStatusSource: "system"`，不伪装成模型生成；模型格式或连接故障仍按原错误路径处理。日志记录 `socialIntent`、`modelAnswerStatus` 和 `socialFallback`，前台继续使用 `speechText` 播报。

“你好，请问门票多少钱”含有业务问题，不会被问候关键词截断或绕过知识约束。无需往知识库补录问候，也无需切换为“允许补充一般知识”。当前只覆盖明确的常见社交短句，不是通用闲聊意图分类器。
...
问答时，小知识库在 24,000 字符预算内全量提供给模型，仅调用一次模型。超过预算后先使用同一已配置模型生成同义问法和检索关键词，再对原始资料排序，最多选取 12 个文件片段。改写只帮助检索，不作为事实证据或播报内容；严格知识库模式下仍无匹配时直接使用知识不足话术，不把任意前 12 片当作证据。

大库每轮最多增加一次短模型请求（最多 400 输出 Tokens、5 秒或更短的已配置超时），会增加少量费用与延迟，无须新增 Key 或外部服务。改写失败时，如果原问题仍能匹配资料则降级继续回答，否则使用自然服务兜底。日志中的 `question.retrieval`、`modelStage`、`rewriteStatus` 和改写错误字段用于区分两步调用。此方案提升同义召回，但不保证大型知识库任意问法都召回，真实模型质量仍需专项验收。文件名不作为访客端“资料来源”展示。

PDF 只导入实际文字层；空白或纯图片 PDF 会提示先 OCR，不再把解析器页码当成知识。多页混合 PDF 只导入有文字的页面，图片页内容不会自动识别。

主持停止独立于播报请求；控制与稿件保存请求 10 秒未确认时会释放操作锁并提示核对。保存期间新增编辑保留为未保存草稿；保存回包丢失时只在服务器稿件与本次提交一致后确认版本，不覆盖其他管理页修改。新管理页给 `POST /api/live-control/present` 和 `/mode` 携带 `expectedInstanceId`、`expectedSequence`，旧指令遇到停止、模式变化或重启时返回 `409 LIVE_CONTROL_STALE_COMMAND`，不会自动重播。旧 API 的单字段请求仍兼容；外部控制端也应携带版本字段并在 409 后刷新状态。
...
语音实现请区分仓库默认配置与扣子部署：仓库默认仍为 `browser`；前端现已实现 `speechProviders.server`，通过扣子已有的 `POST /api/tts` 获取音频。扣子已经匹配的内置 TTS、ASR、男声音色和服务端配置不需要重做。部署这次“打字中断／换声”修复时，请按 [扣子语音修复交接](COZE_SPEECH_FIX.md) 定向更新，不能用仓库的浏览器 ASR 版本覆盖扣子现有的 `ServerVoiceInput`。

前台使用四段带背景的 H.264 MP4 表现四种状态，统一为 720×960、30fps、无音轨；四段合计约 2.03 MB。主持状态使用 2026-09-13 的独立 5 秒片段（约 0.55 MB），不再与对话共用动作。手机首屏只加载待机与真人海报（约 0.61 MB），普通网络在思考期间再预装说话，省流量网络按当前姿态加载。

```text
对话模式：提交问题 → thinking → 模型答案就绪（仍思考）→ 音频开始 → speaking → idle
...
当前播报使用浏览器语音合成，从 `public/avatar-config.json` 的 `speech.preferredVoiceNames` 指定男声名单中选择：macOS/Chrome 首选 `Reed`，再依次尝试 `Eddy`、`Rocko` 以及云希、云健、云扬、康康等。选中的音色在当前页面固定；声音列表重排或暂时缺失都不会改用女声或未指定音色。首次加载或指定男声暂不可用时最多等待 1.5 秒，仍不可用则保留文字并提示，不调用系统默认声音。浏览器 API 没有性别字段，新增名单项需人工确认实际音色；跨设备统一音色仍需接入服务端 TTS。语速、音高仍在 `speech` 中配置。[浏览器声音属性](https://developer.mozilla.org/en-US/docs/Web/API/SpeechSynthesisVoice)

生成回答、等待音频和播报过程中，文字框仍可编辑下一题，发送、麦克风、快捷问题和四态测试按钮暂不可用；Enter 或重复点击不会取消当前轮次，也不会清空草稿。当前回答结束后恢复发送，由用户确认发送，不自动排队提交。中文输入法确认键不触发提问。主动静音、后台主持接管/停止和控制断线仍保留明确的中止语义。

语音失败、缺少指定男声或启动超时会恢复提问。若浏览器开始播报却丢失结束事件，播放看门狗按文本长度和语速给出宽裕预算（至少 60 秒）后恢复控件，避免永久锁住；旧回调不能结束新一轮。录音会话也隔离迟到事件，避免中止后的识别结果覆盖草稿。

视频不可用或用户启用“减少动态”时，保留同一男性数字人的静态海报，问答仍可使用。播放被拦截时可点击“播放人物”，加载失败时可点击“重新加载”。

手机布局保留首屏输入框，对话内容独立滚动，适配安全区、键盘可见高度和横屏；没有初始欢迎对话或资料来源提示。MP4 不再要求 Alpha 解码，但微信各版本的播放和语音策略仍需目标手机实测。
...
2026-09-15 补充：`server` 播放器已实现有界合成等待、Blob 播放、实际 `playing` 驱动姿态和按轮次清理。音频解锁仅在空闲用户手势下运行，同一时刻只允许一个解锁；晚到回调不能改写正式音频。无全局按键解锁，服务端音频失败也不会调用浏览器语音。页面 CSP 仅增加 `media-src` 的 `blob:`，不放行 `data:` 音频。仓库没有新增平台 TTS/ASR 后端，默认配置保持不变。

可交给其他 AI 执行的接入设计、代码地图、建议接口和验收清单见 [外部模型接入与 AI 开发交接说明](EXTERNAL_MODEL_INTEGRATION.md)。其中建议新增的服务端语音接口尚未实现。
...
- 第三方 API Key 只保存在服务端；浏览器调用本项目的语音代理接口，不直接持有供应商密钥。这不要求恢复已撤除的管理来源限制。
- 浏览器识别引擎可能使用厂商的远端服务，不应把它视为必然的本地处理；涉及学生或访客敏感信息时，应按隐私要求选择并配置正式 ASR 服务。
- 选择服务端 TTS 后，失败、超时或播放拦截均保留文字并恢复输入，不自动切换浏览器语音；浏览器 provider 只能显式选择。相同音色与文本可按哈希缓存，减少等待时间和调用成本。
...
预览页保留四个测试按钮，并显示播放器状态与错误原因。视频映射和重建方法见 `public/avatar-media/README.md`；数字人名称、海报和素材路径在 `public/avatar-config.json` 中配置，主持词统一在 Web 工作台维护。旧样例快捷问题已停用。前台缓存当前服务异常话术，请求断连或代理响应无效时仍会显示并尝试播报；实际发声仍取决于浏览器语音能力和声音开关。

主持控制使用实例 ID、指令序号和当前播放指令号同步；过期状态不会覆盖新指令，重连可以获知已错过的停止。控制通道断线时暂停旧播报，不自动重放，需后台重新下发。健康检查不再改变 SSE 控制顺序。语音失败、取消、静音不再显示“播报完成”。

前台问答的响应头及正文读取共用 140 秒最终截止，覆盖最长模型预算；一般网络报错立即兜底，持续挂起则到期兜底并恢复发送。主动取消和主持接管不会误报超时，迟到回答不会重新播报。超时日志为 `CLIENT_REQUEST_TIMEOUT`，无需新增配置。
...
审查时使用 `npm run test:review`，它将已确认问题的 TODO 断言作为普通失败执行。`npm test` 显示的 TODO 不属于通过项；详见 [补测与审查记录](TEST_REVIEW_20260912.md)。

2026-09-15 问候修复后的普通与严格回归均为 261/261 通过、0 TODO；在扣子播放修复的 254 项基础上新增 7 组社交路由回归，覆盖空/小/大库、业务混合句、误拒答、故障日志、主持接管和真实本机 HTTP。此前播报交互、主持素材与恢复性测试仍保留，详见 [测试指南](TESTING.md)；[第三轮复测报告](FIX_REVIEW_20260913_R3.md)、[第二轮并发/取消修复](FIX_REVIEW_20260913_R2.md) 与 [第一轮格式/传输修复](FIX_REVIEW_20260913.md) 保留为历史记录。测试通过不等于真实模型、设备和部署均已验收。

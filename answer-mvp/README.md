...
这是一个可运行的双模式原型：对话模式把访客问题与知识库中的人工知识、文件知识交给后台配置的大语言模型；主持模式由后台选择预先写好的主持词，实时通知所有已打开的数字人前台原样播报。
...
- `http://127.0.0.1:8080/`：双模式控制、人工知识、文件知识与模型配置工作台。
...
- 未登录时，管理地址只显示密码页；人工知识、文件知识和模型配置接口同时由服务端拦截。
...
- `仅依据后台内容`：模型只能基于人工知识与已导入的文件知识回答；资料不足时返回后台配置的“知识不足话术”。
- `允许补充一般知识`：优先使用后台内容，可以补充通用知识，但不得编造本项目专属的日期、地点、费用、人员或规则。

无论选择哪种模式，人工知识和文件知识都只是模型的知识上下文，不会被接口直接当作最终答案返回。
...
- **对话模式**：访客可以文字或语音提问，服务调用已配置的大语言模型，并结合人工知识和文件知识生成答案。
...
每条日志包含时间、动作、请求编号、接口路由、操作者类型、客户端 IP、HTTP 状态码、执行耗时、错误码和必要的计数/ID。系统明确不记录密码、Cookie、Authorization、API Key、系统提示词、回答风格、兜底话术、提问正文、回答正文、知识文件正文或主持词正文。
...
- 新增、编辑、复制、排序、删除和搜索人工知识。
- 为每条人工知识填写适用问题、检索关键词和知识内容。
...
问答时服务会根据问题检索相关文件片段，最多取 12 个并受总上下文长度限制。文件名只在管理页显示，不会作为访客端的“资料来源”展示。人工知识与文件知识共同构成对话模式的知识库。
...
视频替换方法见 `public/avatar-media/README.md`；数字人名称、欢迎语和素材路径在 `public/avatar-config.json` 中配置，主持词统一在 Web 工作台维护。访客快捷问题取自 `content.json` 排名前三条内容的第一种问法；在工作台调整内容或顺序后，访客页会按内容版本自动刷新。
...
每条人工知识需要唯一 `id`、非空 `questions`、非空 `keywords` 和 `answer`。界面分别显示为知识 ID、适用问题、检索关键词和知识内容；底层字段保持不变，以继续兼容现有 `content.json`。
...
当前提交的 37 项测试覆盖管理密码首次设置、跨来源登录及后台操作、退出、加盐哈希、运维日志持久化/筛选/下载/脱敏/轮转、主持词持久化、模式切换、确定性 SSE 指令、主持模式问答互斥、知识文件导入/去重/替换/删除、重启恢复、DOCX/PDF 文字提取、知识上下文检索、候选模型配置失败回滚、Key 不回显、上游错误脱敏、内容热加载、数字人状态机和视频 Range 请求。

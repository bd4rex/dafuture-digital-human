...
当前播报使用浏览器本地语音合成，并优先选择普通话男声：macOS/Chrome 首选 `Reed`，再依次尝试 `Eddy`、`Rocko` 以及 Windows 常见的云希、云健、云扬、康康等男声。语速和音高在 `public/avatar-config.json` 的 `speech` 字段中配置。浏览器没有这些音色时会降级到本机可用的中文语音，因此正式部署如需跨设备保持同一音色，应接入服务端 TTS。视频不可用或用户启用“减少动态”时，保留同一男性数字人的静态海报，问答仍可使用。播放被拦截时可点击“播放人物”，加载失败时可点击“重新加载”。
...
2026-09-13 主持素材更新后的普通与严格回归均为 228/228 通过、0 TODO，新增主持独立素材与海报缓存版本检查。此前 227 项涵盖后台挂起后的有界恢复、部分日志写入、队列旧确认、语音失败提示和测试清理顺序，详见 [第三轮复测报告](FIX_REVIEW_20260913_R3.md)；[第二轮并发/取消修复](FIX_REVIEW_20260913_R2.md) 与 [第一轮格式/传输修复](FIX_REVIEW_20260913.md) 保留为历史记录。测试通过不等于真实模型、设备和部署均已验收。

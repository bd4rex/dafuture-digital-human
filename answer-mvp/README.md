...
- `仅依据后台内容`：模型只能基于人工知识与已导入的文件知识回答；资料不足时返回“当前内容中暂无相关信息。”。
...
每条日志包含时间、动作、请求编号、接口路由、操作者类型、客户端 IP、HTTP 状态码、执行耗时、错误码和必要的计数/ID。系统明确不记录密码、Cookie、Authorization、API Key、系统提示词、提问正文、回答正文、知识文件正文或主持词正文。

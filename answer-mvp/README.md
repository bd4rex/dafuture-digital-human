...
这是一个可运行的双模式原型：对话模式把访客问题与手工问答、外部文件知识库交给后台配置的大语言模型；主持模式由后台选择预先写好的主持词，实时通知所有已打开的数字人前台原样播报。
...
- `http://127.0.0.1:8080/`：双模式控制、内容、知识库与模型配置工作台。
...
- **对话模式**：访客可以文字或语音提问，服务调用已配置的大语言模型，并结合手工内容和外部知识文件生成答案。
...
- 手工内容保存、知识文件导入/删除/下载、模型配置保存和连接测试。
...
第一次打开管理页时直接设置密码，之后使用该密码登录。首次设置和登录接口均不校验页面来源；登录后的会话管理接口仍执行同源检查。`ADMIN_PASSWORD` 仅作为可选的预设方式，不是远程访问的前置条件。管理密码与模型服务的 API Key 是两个独立凭据；如需给自动化程序另行授权，再单独设置 `ADMIN_API_KEY`。
...
当前提交的 37 项测试覆盖管理密码首次设置、跨来源登录、退出、加盐哈希和登录后管理接口同源保护、运维日志持久化/筛选/下载/脱敏/轮转、主持词持久化、模式切换、确定性 SSE 指令、主持模式问答互斥、知识文件导入/去重/替换/删除、重启恢复、DOCX/PDF 文字提取、知识上下文检索、候选模型配置失败回滚、Key 不回显、上游错误脱敏、内容热加载、数字人状态机和视频 Range 请求。
...
启动后打开管理页设置密码。命名卷同时持久化手工内容、主持词、运维日志、模型配置、管理密码哈希、知识索引和原文件。不要把含真实 Key 或运行日志的卷导出到公开位置。

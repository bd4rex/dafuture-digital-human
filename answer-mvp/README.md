...
前台使用四段带背景的 H.264 MP4 表现四种状态，统一为 720×960、30fps、无音轨；四段合计约 1.88 MB。手机首屏只加载待机与真人海报（约 0.61 MB），普通网络在思考期间再预装说话，省流量网络按当前姿态加载。
...
2026-09-13 最新修复及回归为 227/227 通过、0 TODO，覆盖后台挂起后的有界恢复、部分日志写入、队列旧确认、语音失败提示和测试清理顺序。详见 [第三轮复测报告](FIX_REVIEW_20260913_R3.md)；[第二轮并发/取消修复](FIX_REVIEW_20260913_R2.md) 与 [第一轮格式/传输修复](FIX_REVIEW_20260913.md) 保留为历史记录。测试通过不等于真实模型、设备和部署均已验收。

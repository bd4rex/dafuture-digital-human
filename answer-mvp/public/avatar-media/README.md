...
- Chromium / Edge 使用 VP9 Alpha WebM，四段合计约 6.1 MB。
- Safari 使用 HEVC Alpha MOV，四段合计约 9.9 MB。
...
- 原“对话模式”母版只有微笑、眨眼和轻微转头，几乎没有开口动作，视觉上容易与待机状态混淆。当前 `speaking.webm` / `speaking.mov` 临时复用口型明显的主持片段，但仍由独立的 `speaking` 状态和音频事件控制。取得新的对话母版后应重新生成这两个文件；使用旧对话母版运行构建脚本会覆盖这一临时修正。

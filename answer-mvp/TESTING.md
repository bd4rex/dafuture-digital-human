...
- 当前五项 `REVIEW-*` 回归已修复并转为普通测试，174 项全部通过、0 TODO；见 [修复与复测报告](FIX_REVIEW_20260913.md) / [English](FIX_REVIEW_20260913.en.md)。
...
| `test/avatar-runtime.test.js` | 执行前台源码：SSE 断线/重连、语音超时和晚回调、日志重试、请求悬挂 | 隔离 VM 与假时钟，不验证真实设备可听见声音 |
| `test/workbench-runtime.test.js` | 执行工作台源码：控件事件、保存/停止竞态、超时及晚回包、未确认保存恢复 | 隔离 VM，不代替浏览器布局或真机验收 |
